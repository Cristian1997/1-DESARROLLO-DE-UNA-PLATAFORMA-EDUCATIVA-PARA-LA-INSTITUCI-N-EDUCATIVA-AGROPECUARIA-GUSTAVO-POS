...
function guardar_asistencias() {
    ...
    tabla_asistencias.rows().every(function () {
        var rowData = this.data();
        var id_asistencia = rowData.id_asistencia;
        var asistencia = this.nodes().to$().find('.asistencia-checkbox').is(':checked') ? 'ASISTIÓ' : 'NO ASISTIÓ';
        var dia = rowData.dia;
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (resp == 1) {
                    Swal.fire("Mensaje De Confirmacion", "Asistencia guardada correctamente", "success")
                        .then((value) => {
                            
                        });
                } else {
                    Swal.fire("Mensaje De Error", "Lo sentimos, no se pudo guardar la asistencia para ID: " + id_asistencia, "error");
                }
            },
            error: function (xhr, status, error) {
                console.error("Error en la petición AJAX: " + error);
                Swal.fire("Mensaje De Error", "Error al procesar la solicitud", "error");
            }
        });
    });
}
...
function Editar_asistencias(){
    ...
    if (dia.length == 0 || asistencia.length == 0) {
        return Swal.fire("Mensaje De Advertencia","llebes los Datos Vacios","warning");
    }
    ...
    $.ajax({
        ...
        if (resp > 0) {
            if (resp == 1) {
                ...
                Swal.fire("Mensaje De Confirmacion", "asistencias Actualizada correctamente", "success")
                .then((value) => {
...
function actualizar_asistencias() {
    ...
    $.ajax({
        ...
        if (resp > 0) {
            ...
            Swal.fire("Mensaje De Error", "Lo sentimos, no se realizó la actualización", "error");
        }
    });
}
