...
function listar_chat_estudiantes() {
    ...
    columns: [
...

      {
        ...
        render: function (data, type, row) {
          return (
            '<img class="img-responsibe img-circle elevation-2" style="width:60px" src="../' +
            row.foto_usuario_principal +
            '">' +
            "&nbsp;&nbsp;&nbsp;Inicio un chat con&nbsp;&nbsp;&nbsp;" +
            '<img class="img-responsibe img-circle elevation-2" style="width:60px" src="../' +
            row.foto_usuario_secundario +
            '">'
          );
        },
      },
      ...
      {
        defaultContent:
          "<button style='font-size:13px;' type='button' title='Haz Un Comentario Aqui!' class='abrir_chat btn btn-primary'><i class='fa fa-commenting-o'></i></button>",
      },
    ],
  ...
  document.getElementById("tabla_chat_estudiantes_filter").style.display =
    "none";
...
function nuevo_chat_estudiantes() {
  $("#modal_nuevo_chat_estudiantes").modal({
    backdrop: "static",
    keyboard: false,
  });
...
function nuevo_chat_docentes() {
  $("#modal_nuevo_chat_docentes").modal({
    backdrop: "static",
    keyboard: false,
  });
...
$(document).ready(function () {
  $(document).on("click", ".abrir_imagen", function () {
    ...
    $("#modal_archivo_imagen_ver .modal-body").html(
      '<img src="' + imageUrl + '" style="max-width: 100%; height: auto;">'
    );

    $("#modal_archivo_imagen_ver").modal({
      backdrop: "static",
      keyboard: false,
    });
...
function responder_chat() {
  ...
  if (!id_chat || !comentario || !id_usuario) {
    Swal.fire(
      "Mensaje De Error",
      "Seleccione un chat y escriba un mensaje",
      "warning"
    );
    return;
  }
    ...
    if (resp != "") {
      if (resp == -1) {
        Swal.fire(
          ...
          "Ya existe un chat iniciado con este usuario. Por favor, seleccione otro usuario.",
          "warning"
        );
        return;
      }
      ...
      if (resp == 1) {
        Swal.fire(
          ...
          "Datos correctamente, Nuevo Chat Registrado",
          "success"
        ).then((value) => {
...
function enviar_archivo() {
  ...
  if (extensionesPermitidas.indexOf(extension) === -1) {
    return Swal.fire(
      "Mensaje De Advertencia",
      "Solo se permiten archivos PDF, Word, Excel y PowerPoint",
      "warning"
    );
  }
...
function enviar_imagen() {
  ...
  if (fileSize > maxSize) {
    return Swal.fire(
      "Mensaje De Advertencia",
      "La imagen supera el límite de tamaño de 2 MB",
      "warning"
    );
  }
  ...
  if (extensionesPermitidas.indexOf(extension) === -1) {
    return Swal.fire(
      "Mensaje De Advertencia",
      "Solo se permiten archivos de imagen JPG, JPEG, PNG y GIF",
      "warning"
    );
  }
...
$(document).on("click", ".abrir_chat", function () {
  ...
  var imagenUsuario = $(this)
    .closest("li")
    .find(".contacts-list-img")
    .attr("src");
    ...
    if (data.data.length > 0) {
      ...
      data.data.forEach(function (conversacion, index) {
        ...
        listaConversacionesHTML +=
          ...
          '<span class="contacts-list-name" style="display: inline-block;">' +
          nombreUsuario +
          "</span>" +
          '<small class="contacts-list-date pull-right" style="display: inline-block; vertical-align: middle;">' +
          '<button style="font-size: 13px;" type="button" title="Haz Un Comentario Aquí!" class="abrir_chat btn btn-primary" data-id="' +
          conversacion.ID +
          '"><i class="fa fa-commenting-o"></i></button>' +
          "</small>" +
          "</div>" +
          "</a>" +
          '<span class="contacts-list-msg" style="margin-left: 6px;">' +
          mensaje +
          "</span>" +
          "</li>";
    ...
    if (data.data && data.data.length > 0) {
      data.data.forEach(function (comentario) {
        ...
        comentarioHTML +=
          ...
          (esUsuarioActual ? "right" : "left") +
          '">' +
          comentario.nombre +
          "</span>";
        comentarioHTML +=
          '<span class="direct-chat-timestamp pull-' +
          (esUsuarioActual ? "left" : "right") +
          '">' +
          fechaFormateada +
          "</span>";
        comentarioHTML += "</div>";
        ...
        if (comentario.archivo) {
          ...
          if (/\.(jpeg|jpg|png|gif)$/i.test(nombreArchivo)) {
            comentarioHTML +=
              '<img src="../' +
              comentario.archivo +
              '" alt="Archivo adjunto" style="max-width: 100px;">'; // Ajusta el valor de max-width
            comentarioHTML += '<div class="clearfix">';
            comentarioHTML += '<div class="pull-right">';
            comentarioHTML +=
              '<button type="button" class="abrir_imagen btn btn-primary btn-sm btn-flat">Abrir</button>';
            comentarioHTML += "</div>";
            comentarioHTML += "</div>";
          } else {
            comentarioHTML +=
              ...
              '" download="' +
              nombreArchivo +
              '" style="color: black; font-weight: bold; text-decoration: none;">Descargar archivo</a>';
          }
      ...
      $(".direct-chat-messages").html(
        '<div class="alert alert-warning">Aún no se ha iniciado una conversación.</div>'
      );
    }
    ...
    if (data.data.length > 0) {
      data.data.forEach(function (integrante) {
        var usuarioHTML =
          ...
          "</span>" +
          '<span class="users-list-date"><button type="button" title="Iniciar un chat con este usuario!" class="iniciar_chat btn btn-success" data-id="' +
