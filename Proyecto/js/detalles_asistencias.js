...
function listar_detalles_asistencias(id_grado, id_asignatura, id_estudiantes) {
  ...
  tabla_detalles_asistencias = $("#tabla_detalles_asistencias").DataTable({
    ...
    columns: [
      ...
      {
        defaultContent:
          "<button style='font-size:13px; margin-right: 1px;' type='button' class='modal_ver_asistencia btn btn-default' title='Ver asistencias de estudiante' ><i class='fa fa-eye'></i></button>",
      },
    ],
    language: idioma_espanol,
    select: true,
  });
  document.getElementById("tabla_detalles_asistencias_filter").style.display =
    "none";
  ...
  $("#tabla_detalles_asistencias").on(
    "click",
    ".modal_ver_asistencia",
    function () {
...
function listar_tabla_detalles_asistencias_ES() {
  ...
  document.getElementById(
    "tabla_detalles_asistencias_ES_filter"
  ).style.display = "none";
  $("input.global_filter").on("keyup click", function () {
    filterGlobal();
  });
