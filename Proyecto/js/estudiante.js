    ...
    if (resp > 0) {
      if (resp == 1) {
        Swal.fire(
          ...
          "Datos correctamente, Nuevo Estudiante Registrado",
          "success"
        ).then((value) => {
          LimpiarRegistro();
        ...
        return Swal.fire(
          "Mensaje De Advertencia",
          "Lo sentimos, el nombre del estudiante ya se encuentra en nuestra base de datos",
          "warning"
        );
      }
    } else {
...
function Editar_Estudiante() {
  ...
  if (
    nombre.length == 0 ||
    apellido.length == 0 ||
    documentoactual.length == 0 ||
    documentonuevo.length == 0 ||
    telefono.length == 0 ||
    fecha.length == 0 ||
    grado.length == 0 ||
    email.length == 0 ||
